...
class ModelArgsAssertions:
    """Assertions for text encoder model arguments."""
    @staticmethod
    def validate(model_args) -> None:
        ...
        if not model_args.use_diffusion:
            warnings.warn(
                "use_diffusion must be True for image generation models, set use_diffusion=True "
                "if utilizing image generation."
            )
        ...
        if model_args.enable_mqa and model_args.query_groups != 1:
            warnings.warn(
                "Got enable_mqa=True, but query_groups != 1, setting attention to GQA. "
                "To use MQA, set enable_mqa=True AND query_groups = 1."
            )

        if not model_args.enable_mqa and model_args.query_groups == 1:
            warnings.warn(
                "Got query_groups=1, but enable_mqa=False, setting attention to GQA "
                "To use MQA set query_groups = 1 AND enable_mqa=True"
            )
