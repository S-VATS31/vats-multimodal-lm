...
class VQVAEDecoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        with autocast(device_type=device.type, dtype=dtype):
            ...
            B, H, W, _ = x.shape
            assert x.size(-1) == self.d_model, f"expected last dim {self.d_model}, got {x.size(-1)}"

            # Reshape to [B, d_model, H, W] for ConvTranspose2d
            x = x.permute(0, 3, 1, 2).contiguous()
            assert x.shape == (B, self.d_model, H, W), f"expected {(B, self.d_model, H, W)}, got {x.shape}"
